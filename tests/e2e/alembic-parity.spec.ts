...
for (const mode of ["sqlite", "postgresql"] as const) {
  test(`runs the complete ${mode} Alembic command and inspection flow`, async ({ page }) => {
    ...
    if (mode === "postgresql") {
      await page.evaluate(() => {
        window.__revisionLabT3 = window.__revisionLabT5;
      });
    }
    ...
    expect(initial).toMatchObject({
      files: [],
      revisions: [],
      schema: { dialect: mode, tables: [], alembicVersion: [] },
    });
    ...
    expect(initialized.fileChanges.map((change) => change.path)).toEqual(
      expect.arrayContaining(["alembic.ini", "alembic/env.py", "alembic/script.py.mako", "models.py"]),
    );
    ...
    const firstDisposable = await page.evaluate(() =>
      window.__revisionLabT3!.runAlembic(["revision", "-m", "disposable parent", "--rev-id", "discard1"]),
    );
    const secondDisposable = await page.evaluate(() =>
      window.__revisionLabT3!.runAlembic(["revision", "-m", "disposable head", "--rev-id", "discard2"]),
    );
    const firstDisposablePath = firstDisposable.fileChanges.find((change) => change.path.includes("/versions/"))!.path;
    const secondDisposablePath = secondDisposable.fileChanges.find((change) =>
      change.path.includes("/versions/"),
    )!.path;
    const nonHeadDelete = await page.evaluate(async (path) => {
      try {
        await window.__revisionLabT3!.deleteRevision(path);
        return null;
      } catch (error) {
        return (error as { fault?: { code?: string } }).fault ?? null;
      }
    ...
    const deletedHead = await page.evaluate(
      (path) => window.__revisionLabT3!.deleteRevision(path),
      secondDisposablePath,
    );
    ...
    expect(
      (await page.evaluate((path) => window.__revisionLabT3!.deleteRevision(path), firstDisposablePath)).state
        .revisions,
    ).toEqual([]);
    const ordinaryFileDelete = await page.evaluate(async () => {
      try {
        await window.__revisionLabT3!.deleteRevision("models.py");
        return null;
      } catch (error) {
        return (error as { fault?: { code?: string } }).fault ?? null;
      }
    ...
    const manual = await page.evaluate(() =>
      window.__revisionLabT3!.runAlembic(["revision", "-m", "create users", "--rev-id", "001"]),
    );
    ...
    await page.evaluate(({ path, content }) => window.__revisionLabT3!.writeFile(path, content), {
      path: manualPath!,
      content: revision(
        "001",
        null,
    ...
    expect(
      upgraded.after.schema.tables.find((table) => table.name === "users")?.columns.map((column) => column.name),
    ).toEqual(["id", "name"]);
    ...
    await page.evaluate(() =>
      window.__revisionLabT3!.writeFile(
        "models.py",
...
`,
    ...
    const generated = await page.evaluate(() =>
      window.__revisionLabT3!.runAlembic(["revision", "--autogenerate", "-m", "add email", "--rev-id", "002"]),
    );
    ...
    expect(await page.evaluate((path) => window.__revisionLabT3!.readFile(path!), generatedPath)).toContain(
      "add_column",
    );
    ...
    expect(generatedUpgrade.schemaDiff.changes).toContainEqual(
      expect.objectContaining({ kind: "column", table: "users", name: "email", change: "added" }),
    );
    expect(generatedUpgrade.after.schema.tables[0]?.columns.map((column) => column.name)).toEqual([
      "id",
      "name",
      "email",
    ]);
    expect(
      (await page.evaluate(() => window.__revisionLabT3!.runAlembic(["downgrade", "001"]))).after.schema.alembicVersion,
    ).toEqual(["001"]);
    expect(
      (await page.evaluate(() => window.__revisionLabT3!.runAlembic(["upgrade", "head"]))).after.schema.alembicVersion,
    ).toEqual(["002"]);

    expect(
      (
        await page.evaluate(() =>
          window.__revisionLabT3!.runAlembic(["revision", "-m", "alice", "--head", "002", "--rev-id", "a1"]),
        )
      ).success,
    ).toBe(true);
    expect(
      (
        await page.evaluate(() =>
          window.__revisionLabT3!.runAlembic(["revision", "-m", "bob", "--head", "002", "--splice", "--rev-id", "b1"]),
        )
      ).success,
    ).toBe(true);
    for (const [id, column] of [
      ...
      const content = revision(
        id!,
        "002",
        `op.add_column("users", sa.Column("${column}", sa.String(40)))`,
    ...
    expect(upgradedHeads.after.schema.tables[0]!.columns.map((column) => column.name).sort()).toEqual([
      "alice_note",
      "bob_note",
      "email",
      "id",
    ...
    expect(
      upgradedHeads.after.revisions
        .filter((node) => node.isCurrent)
        .map((node) => node.revision)
        .sort(),
    ).toEqual(["a1", "b1"]);
    const merged = await page.evaluate(() =>
      window.__revisionLabT3!.runAlembic(["merge", "a1", "b1", "-m", "merge branches", "--rev-id", "m1"]),
    );
    expect(merged.success, merged.traceback).toBe(true);
    expect(merged.after.revisions.find((node) => node.revision === "m1")).toMatchObject({
      downRevisions: ["a1", "b1"],
      isHead: true,
      isMergePoint: true,
    });
    ...
    const failedRevision = await page.evaluate(() =>
      window.__revisionLabT3!.runAlembic(["revision", "-m", "broken SQL", "--rev-id", "fail1"]),
    );
    ...
    await page.evaluate(({ path, content }) => window.__revisionLabT3!.writeFile(path, content), {
      ...
      content: revision(
        "fail1",
        "m1",
        'op.create_table("rollback_probe", sa.Column("id", sa.Integer(), primary_key=True))\nop.execute("THIS IS NOT SQL")',
      ),
    });
    ...
    expect(rejected).toMatchObject({
      success: false,
      error: { code: "UNSUPPORTED_ALEMBIC_COMMAND" },
    });
    const rejectedOption = await page.evaluate(() => window.__revisionLabT3!.runAlembic(["upgrade", "--sql", "head"]));
    expect(rejectedOption).toMatchObject({
      success: false,
      error: { code: "UNSUPPORTED_ALEMBIC_OPTION" },
    });
    const concurrent = await page.evaluate(async () => {
      ...
      return requests.map((result) =>
        result.status === "fulfilled" ? "OK" : (result.reason as { fault: { code: string } }).fault.code,
      );
    });
...
test("runs SQLite without cross-origin isolation", async ({ page }) => {
  ...
  const initialized = await page.evaluate(() => window.__revisionLabT3!.runAlembic(["init"]));
  expect(initialized.success, initialized.traceback).toBe(true);
  expect(initialized.after.schema).toMatchObject({
    dialect: "sqlite",
    tables: [],
    alembicVersion: [],
  });
