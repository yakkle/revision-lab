...
test("executes DDL, parameterized data, errors and 100 serial calls through Python", async ({ page }) => {
  ...
  expect(report.databaseError).toMatchObject({
    ok: false,
    error: { code: "PGLITE_DATABASE_ERROR", sqlState: "23505" },
  });
...
test("bounds oversized responses and remains usable", async ({ page }) => {
  ...
  const oversized = await page.evaluate(() =>
    window.__revisionLabT2!.query("SELECT repeat('x', 9 * 1024 * 1024) AS large"),
  );
...
test("reconnects an idle runtime without recreating PGlite", async ({ page }) => {
  ...
  const afterRestart = await page.evaluate(() =>
    window.__revisionLabT2!.query("SELECT id, label FROM reconnect_items"),
  );
    ...
    try {
      ...
      return {
        thrown: {
          ...
          fault: runtimeError.fault,
          stack: runtimeError.stack,
        },
      };
    }
  });
  expect(afterRestart, JSON.stringify(afterRestart)).toMatchObject({
