...
export function LessonGuide({
  ...
  return <section className="lesson-guide" aria-label="학습 가이드">
    <div className="lesson-guide-fixed">
      <div className="lesson-guide-toolbar"><strong>학습 가이드</strong><button onClick={() => lab.guide(false)}>가이드 닫기</button></div>
      <div className="lesson-nav" role="tablist" aria-label="Alembic lesson">
        {lessons.map((item) => {
          ...
          return <button key={item.id} role="tab" aria-label={`${item.number} ${item.title}`} aria-selected={item.id === activeLesson} title={item.title} onClick={() => lab.lesson(item.id)}>
      ...
      <div className="lesson-content">
        <div className="lesson-copy"><div className="lesson-title"><span>{lesson.number}</span><div><h2>{lesson.title}</h2><p>{lesson.concept}</p></div></div>
          ...
          {lesson.example && <details className="lesson-example" open>
            ...
            <div className="example-path">{lesson.example.path && <button disabled={busy || !workspace?.snapshot?.files.includes(lesson.example.path)} onClick={() => void lab.openFile(lesson.example!.path!)}><code>{lesson.example.path}</code> 열기</button>}<span>{lesson.example.instruction}</span></div>
            <pre aria-label={`${lesson.example.path ?? lesson.title} 예제 코드`}><code>{lesson.example.code}</code></pre>
          </details>}
