...
import { createLab, MAX_TERMINAL_DOCK_HEIGHT, MIN_TERMINAL_DOCK_HEIGHT, type Lab, type Panel } from "./lab/store";
...
import {
  archiveHasExecutablePython,
  readWorkspaceArchive,
  type WorkspaceArchiveV1,
} from "./persistence/workspace-archive";

declare global {
  interface Window {
    __revisionLabT8?: { crashActive: () => void };
  }
}

const panels: Array<{ id: Panel; label: string }> = [
  { id: "editor", label: "파일 / 코드" },
  { id: "graph", label: "Revision DAG" },
  { id: "database", label: "Schema / Data" },
  { id: "diff", label: "Diff" },
  { id: "logs", label: "원본 로그" },
];
const examples = [
...
export default function App({ controller }: { controller?: Lab }) {
  ...
  const deletionBlockedReason = !selectedRevision
    ? undefined
    : !selectedRevision.isHead
      ? "자식 revision이 있어 삭제할 수 없습니다."
      : selectedRevision.isCurrent || workspace?.snapshot?.schema.alembicVersion.includes(selectedRevision.revision)
        ? "DB에 적용된 revision입니다. 먼저 downgrade하세요."
        : undefined;
  ...
  useEffect(() => {
    const beforeUnload = (event: BeforeUnloadEvent) => {
      ...
      const hasDraft = current.workspaces.some((item) =>
        Object.values(item.drafts).some((itemDraft) => itemDraft.text !== itemDraft.saved),
      );
      if (current.busy || hasDraft) {
  ...
  return (
    <main
      className={`app-shell ${state.guideEnabled ? "guide-open" : "guide-closed"} ${editorMaximized ? "editor-maximized" : ""}`}
      style={{ "--terminal-height": `${state.terminalDockHeight}px` } as CSSProperties}
      onKeyDown={(event) => {
        if ((event.metaKey || event.ctrlKey) && event.key === "s") {
          event.preventDefault();
          if (!disabled) void lab.save();
        }
      }}
    >
      <header className="app-header">
        <a className="brand" href="./">
          ...
          <span>
            <strong>Revision Lab</strong>
            <small>Alembic migration playground</small>
          </span>
        </a>
          ...
          {state.workspaces.length > 0 ? (
            <label>
              ...
              <select
                ...
                disabled={state.busy}
                onChange={(event) => lab.select(event.target.value)}
              >
            ...
            <span className="empty-workspace-label">Workspace 없음</span>
          )}
          <span className={`database-badge ${workspace?.mode ?? ""}`}>
            {workspace ? (workspace.mode === "sqlite" ? "SQLite" : "PostgreSQL") : "DB 미연결"}
          </span>
        </div>
        <div className="workspace-menu" ref={workspaceMenu}>
          <button
            ...
            aria-label="Workspace 관리"
            aria-expanded={openPopover === "workspace"}
            aria-controls="workspace-menu-panel"
            onClick={() => setOpenPopover((current) => (current === "workspace" ? undefined : "workspace"))}
          >
            Workspace 관리
          </button>
          {openPopover === "workspace" && (
            <div className="workspace-menu-panel" id="workspace-menu-panel">
              ...
              <button
                disabled={state.busy || state.restoring || !capabilities.sqliteAvailable || atWorkspaceLimit}
                onClick={() => createWorkspace("sqlite")}
              >
                SQLite workspace 만들기
              </button>
              <button
                disabled={state.busy || state.restoring || !capabilities.postgresqlAvailable || atWorkspaceLimit}
                onClick={() => createWorkspace("postgresql")}
              >
                PostgreSQL workspace 만들기
              </button>
              {!capabilities.postgresqlAvailable && (
                <p className="menu-hint">
                  PostgreSQL에 필요한 기능: {capabilities.postgresqlBlockers.join(", ")}. COOP/COEP 환경에서 사용할 수
                  있습니다.
                </p>
              )}
              <hr />
              <button
                disabled={state.busy || !workspace || dirty || workspace.broken}
                onClick={() => void exportWorkspace()}
              >
                Workspace 내보내기
              </button>
              <button
                disabled={state.busy || atWorkspaceLimit}
                onClick={() => {
                  closeWorkspaceMenu();
              ...
              <details className="environment-details">
                <summary>이 브라우저의 실습 환경</summary>
                <ul>
                  {capabilities.checks.map((check) => (
                    <li key={check.key}>
                      {check.supported ? "✓" : "×"} {check.label} — {check.description}
                    </li>
                  ))}
                </ul>
              </details>
              {workspace && (
                <>
                  ...
                  <button
                    ...
                    onClick={() => {
                      closeWorkspaceMenu();
                      removeWorkspaceDialog.current?.showModal();
                    }}
                  >
                    Workspace 삭제
                  </button>
          ...
          <input
            ...
            aria-label="Workspace archive 파일"
            onChange={(event) => void chooseArchive(event.target.files?.[0])}
          />
        </div>
      </header>
      ...
      <div className="runtime-bar">
        ...
        <span>
          {workspace?.savedAt
            ? `체크포인트 ${new Date(workspace.savedAt).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}`
            : "브라우저 로컬 저장"}
        </span>
      </div>

      <div className="notice-stack">
        {workspace?.recoveredAt && (
          <p className="success-banner" role="status">
            Worker 중단 후 {new Date(workspace.recoveredAt).toLocaleTimeString("ko-KR")}에 마지막 성공 체크포인트로
            복원했습니다.
          </p>
        )}
        {archiveError && (
          <p className="error-box" role="alert">
            <strong>ARCHIVE_IMPORT_REJECTED</strong>
            <br />
            {archiveError}
          </p>
        )}
        {workspace?.error && (
          <section className="error-box" role="alert">
            ...
            {workspace.stale && (
              <p>표시된 snapshot은 마지막으로 확인한 상태입니다. 최신 DB 상태를 확인하지 못했습니다.</p>
            )}
            {workspace.error.traceback && (
              <details>
                <summary>원본 traceback</summary>
                <pre>{workspace.error.traceback}</pre>
              </details>
            )}
            {workspace.broken && (
              <>
                <p>
                  자동 복원이 완료되지 않았습니다. 마지막 성공 체크포인트를 다시 시도하거나 새 workspace를 만드세요.
                </p>
                <button disabled={state.busy} onClick={() => void lab.recover()}>
                  체크포인트 복원 다시 시도
                </button>{" "}
                <button disabled={state.busy || atWorkspaceLimit} onClick={() => void lab.create(workspace.mode)}>
                  새 workspace 만들기
                </button>
              </>
            )}
      ...
      <div className="workbench">
        ...
        <section className="workbench-main" aria-label="Alembic 작업면">
          <nav className="panel-tabs" role="tablist" aria-label="실습 패널">
            {panels.map((panel, index) => (
              <button
                role="tab"
                id={"tab-" + panel.id}
                aria-controls={"panel-" + panel.id}
                aria-selected={state.panel === panel.id}
                tabIndex={state.panel === panel.id ? 0 : -1}
                key={panel.id}
                onClick={() => lab.panel(panel.id)}
          ...
          <div className={`lab-grid showing-${state.panel}`}>
            <section className="panel editor-panel" id="panel-editor" aria-label="파일 / 코드">
              <div className="panel-heading">
                ...
                <div className="panel-actions">
                  <button disabled={disabled || !draft || draft.text === draft.saved} onClick={() => void lab.save()}>
                    파일 저장
                  </button>
                  {selectedRevision && (
                    <button
                      className="danger-action"
                      title={dirty ? "저장하지 않은 편집을 먼저 저장하세요." : deletionBlockedReason}
                      disabled={disabled || dirty || Boolean(deletionBlockedReason)}
                      onClick={() =>
                        setPendingDeletion({
                  ...
                  <button
                    ...
                    onClick={() => (editorMaximized ? restoreEditor() : setEditorMaximized(true))}
                  >
                    {editorMaximized ? "기본 크기로 복원" : "편집기 최대화"}
                  </button>
                </div>
              </div>
              <div className="editor-layout">
                <nav className="file-tree" aria-label="Workspace 파일">
                  {!workspace?.snapshot?.files.length && <p className="empty">init 후 파일이 표시됩니다.</p>}
                  {workspace?.snapshot?.files.map((path) => (
                    <button
                      key={path}
                      disabled={disabled}
                      aria-current={workspace.file === path ? "true" : undefined}
                      onClick={() => void lab.openFile(path)}
                      title={path}
                    >
                      <span>{path}</span>
                      {workspace.drafts[path]?.text !== workspace.drafts[path]?.saved && (
                        <span className="badge">수정</span>
                      )}
                    </button>
                  ))}
                </nav>
                <div className="editor-document">
                  <div className="file-caption">
                    {workspace?.file ?? "파일 선택"}
                    {draft && draft.text !== draft.saved && <span className="badge">저장 안 됨</span>}
                  </div>
                  {workspace?.file && draft ? (
                    <CodeEditor
                      key={workspace.id + ":" + workspace.file}
                      path={workspace.file}
                      value={draft.text}
                      disabled={disabled}
                      onChange={lab.edit}
                    ...
                    <div className="empty editor-empty">
                      <strong>{workspace ? "첫 migration을 만들어 보세요." : "새 실습을 시작하세요."}</strong>
                      <p>
                        {workspace ? (
                          <>
                            <code>alembic init migrations</code>를 실행하면 실제 프로젝트 파일이 생성됩니다.
                          </>
                        ) : (
                          "브라우저 안에서 실제 Alembic과 DB를 실행합니다."
                        )}
                      </p>
                      {!workspace && (
                        <div className="start-actions">
                          <button
                            className="primary"
                            disabled={state.busy || !capabilities.sqliteAvailable}
                            onClick={() => createWorkspace("sqlite")}
                          >
                            SQLite workspace 만들기
                          </button>
                          <button
                            disabled={state.busy || !capabilities.postgresqlAvailable}
                            onClick={() => createWorkspace("postgresql")}
                          >
                            PostgreSQL workspace 만들기
                          </button>
                          {!capabilities.postgresqlAvailable && (
                            <span className="hint">PostgreSQL: {capabilities.postgresqlBlockers.join(", ")} 필요</span>
                          )}
                        </div>
                      )}
                    </div>
            ...
            <section
              className={`panel inspector-panel ${state.panel === "editor" ? "default-graph" : ""}`}
              aria-label="실행 결과"
            >
              <div className="panel-heading">
                <h2>
                  02 / {panels.find((panel) => panel.id === (state.panel === "editor" ? "graph" : state.panel))?.label}
                </h2>
                <span className="badge">{workspace?.snapshot?.schema.dialect ?? "DB 미연결"}</span>
              </div>
              <div className="panel-body">
                <div id="panel-graph" hidden={state.panel !== "editor" && state.panel !== "graph"}>
                  {workspace?.snapshot ? (
                    <RevisionGraph
                      revisions={workspace.snapshot.revisions}
                      selected={workspace.revision}
                      onSelect={(id) => {
                        if (!disabled) void lab.selectRevision(id);
                      }}
                    />
                  ) : (
                    <p className="empty">Workspace를 생성하면 실제 revision graph를 확인할 수 있습니다.</p>
                  )}
                </div>
                <div id="panel-database" hidden={state.panel !== "database"}>
                  {workspace && <DatabasePanel workspace={workspace} lab={lab} disabled={disabled} />}
                </div>
                <div id="panel-diff" hidden={state.panel !== "diff"}>
                  {workspace && <DiffPanel workspace={workspace} lab={lab} disabled={disabled} />}
                </div>
                <div id="panel-logs" hidden={state.panel !== "logs"}>
                  {workspace && <LogsPanel workspace={workspace} />}
                </div>
              </div>
            </section>
          </div>
          ...
          <div
            ...
            aria-valuenow={state.terminalDockHeight}
            tabIndex={editorMaximized ? -1 : 0}
          ...
          <section className="terminal panel" aria-label="Alembic 터미널">
              ...
              <div className="terminal-actions">
                <div className="command-menu" ref={commandMenu}>
                  <button
                    ...
                    aria-controls="terminal-command-examples"
                    onClick={() => setOpenPopover((current) => (current === "commands" ? undefined : "commands"))}
                  >
                    명령 예시
                  </button>
                  {openPopover === "commands" && (
                    <div className="terminal-help" id="terminal-command-examples">
                      <p>예시를 입력한 뒤 실행하세요. pipe, redirection, 외부 shell은 지원하지 않습니다.</p>
                      <div className="command-examples">
                        {examples.map((example) => (
                          <button key={example} onClick={() => stageCommand(example)}>
                            <code>{example}</code>
                          </button>
                        ))}
                      </div>
                ...
                <button onClick={() => lab.panel("logs")}>전체 원본 로그</button>
              </div>
            </div>
            <div className="terminal-output" ref={terminalOutput}>
              {workspace?.entries.slice(-3).map((entry) => (
                <div key={entry.id}>
                  <code>$ {entry.command}</code>
                  <span className={entry.result?.success ? "success-text" : "error-text"}>
            ...
            <form
              onSubmit={(event) => {
                event.preventDefault();
                if (!disabled && !dirty && command.trim()) {
                  setHistoryOffset(0);
                  setCommandNotice(`${command} 명령을 실행하고 있습니다.`);
                  void lab.run(command);
                }
              }}
            >
              ...
              <input
                ref={commandInput}
                id="command"
                aria-label="Alembic 명령"
                value={command}
                spellCheck={false}
                maxLength={8192}
                onChange={(event) => {
                  setCommand(event.target.value);
                  setCommandNotice("명령을 수정했습니다.");
                }}
                onKeyDown={(event) => {
                  ...
                  const next = Math.max(
                    0,
                    Math.min(entries.length, historyOffset + (event.key === "ArrowUp" ? 1 : -1)),
                  );
                  event.preventDefault();
                  setHistoryOffset(next);
                  setCommand(next ? entries[entries.length - next].command : "");
                }}
              />
              <button className="primary" type="submit" disabled={disabled || dirty || !command.trim()}>
                명령 실행
              </button>
            </form>
          </section>
      ...
      {pendingArchive && (
        ...
        >
          ...
          <p id="import-description">
            <strong>{pendingArchive.name}</strong> ·{" "}
            {pendingArchive.archive.workspace.mode === "sqlite" ? "SQLite" : "PostgreSQL"} · 파일{" "}
            {pendingArchive.archive.files.length}개
          </p>
          <p className="warning">
            가져온 migration Python은 브라우저 Worker에서 실제 실행될 수 있습니다. 파일 내용을 확인한 뒤 가져오세요.
          </p>
          <div className="archive-preview">
            <nav aria-label="가져올 파일 목록">
              {pendingArchive.archive.files.map((file) => (
                <button
                  key={file.path}
                  aria-current={previewPath === file.path ? "true" : undefined}
            ...
            <pre aria-label="가져올 파일 내용">
              {pendingArchive.archive.files.find((file) => file.path === previewPath)?.content ?? "파일이 없습니다."}
            </pre>
          </div>
          {archiveHasExecutablePython(pendingArchive.archive) && (
            <label className="review-confirm">
              <input
                type="checkbox"
                checked={pythonReviewed}
                onChange={(event) => setPythonReviewed(event.target.checked)}
              />{" "}
              Python 파일 목록과 내용을 확인했습니다.
            </label>
          )}
          <div className="dialog-actions">
            ...
            <button
              className="primary"
              disabled={archiveHasExecutablePython(pendingArchive.archive) && !pythonReviewed}
              onClick={() => {
      ...
      {pendingDeletion && (
        ...
        >
          ...
          <p id="delete-description">
            <code>{pendingDeletion.path}</code> · revision <code>{pendingDeletion.revision}</code>을 삭제합니다. 삭제 후
            revision graph가 즉시 다시 계산됩니다.
          </p>
          <div>
            <button autoFocus onClick={() => setPendingDeletion(undefined)}>
              취소
            </button>
            <button
              className="danger-action"
              onClick={() => {
                const target = pendingDeletion.path;
                setPendingDeletion(undefined);
                void lab.deleteRevision(target);
              }}
            >
              미적용 migration 삭제
            </button>
      ...
      >
        ...
        <p id="reset-description">
          {state.collaboration && workspace?.collaborationId === state.collaboration.id
            ? "협업 실습의 공통 Base, Alice, Bob, Integration"
            : workspace?.name}
          의 파일과 DB를 삭제하고 처음부터 시작합니다. 저장하지 않은 편집도 사라지며 복구할 수 없습니다.
        </p>
        <div>
      ...
      >
        ...
        <p id="remove-workspace-description">
          {state.collaboration && workspace?.collaborationId === state.collaboration.id ? (
            "협업 실습의 공통 Base, Alice, Bob, Integration workspace와 연결된 파일·DB·체크포인트를 모두 삭제합니다."
          ) : (
            <>
              <strong>{workspace?.name}</strong> · {workspace?.mode === "postgresql" ? "PostgreSQL" : "SQLite"}{" "}
              workspace의 파일·DB·체크포인트를 삭제합니다.
            </>
          )}
          {dirty && " 저장하지 않은 편집도 사라집니다."} 이 작업은 복구할 수 없습니다.
        </p>
        <div>
          <button autoFocus onClick={() => removeWorkspaceDialog.current?.close()}>
            취소
          </button>
          <button
            className="danger-action"
            onClick={() => {
              removeWorkspaceDialog.current?.close();
              void lab.removeWorkspace();
            }}
          >
            Workspace 완전히 삭제
          </button>
