...
function fixture(postMessage: (request: PgRequest, control: Int32Array, response: Uint8Array) => void, timeout = 20) {
  ...
  const port = {
    postMessage: (value: PgRequest) => postMessage(value, control, response),
  } as MessagePort;
  const rpc = new SyncRpc(
    port,
    control,
    response,
    { protocolVersion: PROTOCOL_VERSION, requestId: "test", workspaceId: "test" },
    timeout,
  );
...
describe("synchronous Worker RPC", () => {
  it("returns a matching response", () => {
    const { rpc } = fixture((request, control, response) =>
      publishResponse(control, response, request.sequence, {
        ok: true,
  ...
  it("sends multiple parameter sets as one ordered request", () => {
    ...
    expect(
      rpc.executeMany("INSERT INTO items VALUES ($1)", [[{ tag: "number", value: 1 }], [{ tag: "number", value: 2 }]]),
  ...
  it("marks stale sequences as a protocol error", () => {
    ...
    expect(rpc.query("select 1", [])).toMatchObject({
      ok: false,
      error: { code: "RPC_PROTOCOL_ERROR" },
    });
  ...
  it("replaces an oversized response with a bounded error", () => {
    ...
    expect(rpc.query("select repeat('x', 9000000)", [])).toEqual(
      rpcError("RPC_RESPONSE_TOO_LARGE", `RPC response exceeds ${RESPONSE_BYTES} bytes`),
    );
  });
});
