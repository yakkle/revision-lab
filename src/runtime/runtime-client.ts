...
export class RuntimeClient {
  ...
  async query(sql: string, params: unknown[] = []): Promise<PgResult> {
    ...
    if (!isPgResult(parsed))
      throw new RuntimeClientError({
        code: "RUNTIME_INVALID_RESPONSE",
        message: "Python returned an invalid RPC result",
      });
...
`);
  ...
  private async runPython(source: string): Promise<string> {
    ...
    if (!worker)
      throw new RuntimeClientError({
        ...
        message: "Pyodide Worker is not ready",
      });
  ...
  private async boot(reconnectPglite = false): Promise<void> {
    if (!crossOriginIsolated || typeof SharedArrayBuffer === "undefined") {
      throw new RuntimeClientError({
        ...
        message: "PostgreSQL mode requires cross-origin isolation",
      });
    }
    ...
    const pyodideWorker = new Worker(new URL("./pyodide.worker.ts", import.meta.url), {
      type: "module",
      name: "revision-lab-pyodide",
    });
    const existingPgliteWorker = reconnectPglite ? this.pgliteWorker : undefined;
    const pgliteWorker =
      existingPgliteWorker ??
      new Worker(new URL("./pglite.worker.ts", import.meta.url), {
        type: "module",
        name: "revision-lab-pglite",
      });
  ...
  private async restartWorkers(): Promise<void> {
    this.disconnectPython(
      new RuntimeClientError({
        code: "RUNTIME_RESTARTED",
        message: "Runtime connection restarted",
      }),
    );
    this.starting = undefined;
    ...
    throw lastError instanceof Error
      ...
      : new RuntimeClientError({
          ...
          message: "Runtime Workers could not be restarted",
        });
  }
  ...
  private readonly handlePythonMessage = (event: MessageEvent<unknown>) => {
    ...
    if (value.type === "RESULT" && typeof value.value === "string") pending.resolve(value.value);
      ...
      pending.reject(
        new RuntimeClientError({
          code: "RUNTIME_INVALID_RESPONSE",
          message: "Invalid Python Worker response",
        }),
      );
  };
