import {
  ...
  type PgResult, type PythonReply, type RpcFault, type WorkerBoot,
} from "./protocol";
...
function waitForBoot(worker: Worker, requestId: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = window.setTimeout(() => finish(new RuntimeClientError({ code: "RUNTIME_BOOT_TIMEOUT", message: "Worker boot exceeded 60 seconds" })), 60_000);
...
`);
  ...
  private async boot(): Promise<void> {
    ...
    const pgliteWorker = new Worker(new URL("./pglite.worker.ts", import.meta.url), { type: "module", name: "revision-lab-pglite" });
    ...
    const pyReady = waitForBoot(pyodideWorker, bootId);
    const pgReady = waitForBoot(pgliteWorker, bootId);
    pyodideWorker.postMessage({ ...common, port: channel.port1 }, [channel.port1]);
    pgliteWorker.postMessage({ ...common, port: channel.port2 }, [channel.port2]);
    try {
