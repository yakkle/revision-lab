...
describe("runtime protocol validation", () => {
  it("validates command text, progress and bounded tagged table rows", () => {
    ...
    const data = {
      table: "users",
      columns: ["id"],
      rows: [[{ tag: "bigint", value: "9007199254740993" }]],
      truncated: false,
    };
    ...
    expect(
      isAlembicRuntimeReply({
        ...envelope,
        type: "TABLE_DATA",
        data: { ...data, rows: [[{ tag: "number", value: 9007199254740992 }]] },
      }),
    ).toBe(false);
      ...
      expect(
        isAlembicRuntimeReply({
          ...
          state: {
            files: [],
            revisions: [],
            schema: { dialect, tables: [], alembicVersion: ["a1", "b1"] },
          },
        }),
      ).toBe(dialect !== "mysql");
    }
  ...
  it("validates structured SQLite runtime requests before they reach Python", () => {
    ...
    expect(
      isSqliteRuntimeRequest({
        ...
        path: "migrations/versions/r1.py",
      }),
    ).toBe(true);
    ...
    expect(
      isSqliteRuntimeRequest({
        ...
        seed: {
          files: [{ path: "migrations/versions/base.py", content: "revision = 'base'" }],
          sqliteDatabase: "AA==",
        },
      }),
    ).toBe(true);
    ...
    expect(isSqliteRuntimeRequest({ ...envelope, type: "EXPORT_CLONE" })).toBe(true);
  });
});
