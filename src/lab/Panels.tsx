...
export function LearningNote({ workspace }: { workspace?: Workspace }) {
  ...
  else if (workspace?.snapshot?.files.includes("models.py") && workspace.snapshot.revisions.length === 0) text = "models.py의 기본 User 모델은 수동 migration과 schema 기준을 공유합니다. 먼저 빈 revision에 users 생성·제거 동작을 작성하고 적용한 뒤, 04에서 email 한 줄을 활성화해 변경 migration을 만들어 보세요.";
