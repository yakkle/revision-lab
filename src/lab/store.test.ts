...
const result: CommandResult = {
  ...
  stderr: "",
  fileChanges: [],
  schemaDiff: { changes: [], alembicVersion: { before: [], after: [] } },
};
function client(): LabClient {
  return {
    createWorkspace: vi.fn().mockResolvedValue(snapshot),
    readFile: vi.fn().mockResolvedValue("original"),
    exportClone: vi.fn().mockResolvedValue({
      files: [{ path: "models.py", content: "original" }],
      sqliteDatabase: "",
    }),
    writeFile: vi.fn().mockResolvedValue({ state: snapshot, fileChanges: [] }),
    runCommand: vi.fn().mockResolvedValue(result),
    deleteRevision: vi.fn().mockResolvedValue({ state: snapshot, fileChanges: [] }),
    readTable: vi.fn().mockResolvedValue({ table: "users", columns: [], rows: [], truncated: false }),
    close: vi.fn(),
  };
}

function repository(restored?: {
  ...
  return {
    ...
    loadCheckpoint: vi
      .fn()
      .mockImplementation(async (id: string) => restored?.checkpoints.find((item) => item.workspace.id === id)),
...
describe("Lab workspace orchestration", () => {
  it("resets only the selected workspace and releases its runtime", async () => {
    ...
    const lab = createLab(
      vi.fn().mockReturnValueOnce(first).mockReturnValueOnce(second).mockReturnValueOnce(replacement),
    );
  ...
  it("removes workspaces without creating replacements and selects the nearest remaining workspace", async () => {
    ...
    expect(second.close).toHaveBeenCalledOnce();
    expect(lab.store.getState()).toMatchObject({
      activeId: firstId,
      workspaces: [{ id: firstId }],
    });
    expect(persistence.delete).toHaveBeenLastCalledWith(
      [secondId],
      expect.objectContaining({ workspaceIds: [firstId], activeId: firstId }),
    );
    ...
    expect(persistence.delete).toHaveBeenLastCalledWith(
      [firstId],
      expect.objectContaining({ workspaceIds: [], activeId: undefined }),
    );
    lab.dispose();
  });
  it("blocks duplicate operations and workspace switching until an operation completes", async () => {
    ...
    vi.mocked(runtime.runCommand).mockImplementation(
      () =>
        new Promise((resolve) => {
          finish = resolve;
        }),
  ...
  it("deletes a revision through the runtime, removes its draft, and checkpoints the returned graph", async () => {
    const revisionPath = "migrations/versions/r1.py";
    const revisionState: WorkspaceState = {
      ...snapshot,
      files: ["models.py", revisionPath],
      revisions: [
        {
          revision: "r1",
          path: revisionPath,
    ...
    const runtime = client();
    const persistence = repository();
    vi.mocked(runtime.createWorkspace).mockResolvedValue(revisionState);
    vi.mocked(runtime.readFile).mockImplementation(async (path) =>
      path === revisionPath ? "revision = 'r1'" : "model",
    );
    vi.mocked(runtime.deleteRevision).mockResolvedValue({
      state: snapshot,
      fileChanges: [{ path: revisionPath, change: "deleted" }],
    });
    const lab = createLab(() => runtime, persistence);
    await lab.create("sqlite");
    await lab.openFile(revisionPath);

    await lab.deleteRevision(revisionPath);

    expect(runtime.deleteRevision).toHaveBeenCalledWith(revisionPath);
    expect(lab.store.getState().workspaces[0]).toMatchObject({
      snapshot,
      file: "models.py",
      revision: undefined,
      changes: [{ path: revisionPath, change: "deleted" }],
    });
    expect(lab.store.getState().workspaces[0].drafts[revisionPath]).toBeUndefined();
  ...
  it("uses the actual failed-command snapshot and preserves the original error", async () => {
    ...
    const failed = {
      ...result,
      success: false,
      after: { ...snapshot, schema: { ...snapshot.schema, alembicVersion: ["actual"] } },
      error: { code: "ALEMBIC_COMMAND_FAILED", message: "original SQL error" },
  ...
  it("checkpoints successful commands but never replaces the checkpoint with a failed migration", async () => {
    ...
    await lab.run("alembic heads");
    expect(persistence.save).toHaveBeenCalledTimes(2);
    vi.mocked(runtime.runCommand).mockResolvedValueOnce({
      ...result,
      success: false,
  ...
  it("restores runtime files, database seed, lesson evidence, and the selected workspace from IndexedDB metadata", async () => {
    const seed: RuntimeCloneSeed = {
      files: [{ path: "models.py", content: "restored" }],
      sqliteDatabase: "AA==",
    };
    ...
    const session: PersistedSession = {
      formatVersion: 1,
      workspaceIds: [checkpoint.workspace.id],
      activeId: checkpoint.workspace.id,
      panel: "database",
      guideEnabled: true,
      activeLesson: "autogenerate",
      terminalDockHeight: 248,
    };
    ...
    expect(lab.store.getState().workspaces[0]).toMatchObject({
      savedAt: 1234,
      revision: "saved",
      drafts: { "models.py": { text: "restored", saved: "restored" } },
    });
    lab.dispose();
  });
  ...
  it("clamps and persists the terminal dock height without changing the session format", async () => {
    ...
    expect(vi.mocked(persistence.saveSession).mock.calls.at(-1)?.[0]).toMatchObject({
      formatVersion: 1,
      terminalDockHeight: MAX_TERMINAL_DOCK_HEIGHT,
    });
  ...
  it("retains the last snapshot after a timeout and creates a new runtime only on explicit create", async () => {
    const runtime = client();
    vi.mocked(runtime.runCommand).mockRejectedValue(
      new RuntimeClientError({ code: "ALEMBIC_COMMAND_TIMEOUT", message: "timeout" }),
    );
    const factory = vi.fn(() => runtime);
  ...
  it("clones a real common base and integrates only Alice and Bob revision files", async () => {
    const baseRevision = {
      revision: "base",
      path: "migrations/versions/base.py",
      downRevisions: [],
      branchLabels: [],
      dependsOn: [],
      isHead: true,
    ...
    const baseState: WorkspaceState = {
      files: ["alembic.ini", "models.py", "migrations/env.py", baseRevision.path],
    ...
    for (const runtime of [aliceClient, bobClient, integrationClient])
      vi.mocked(runtime.createWorkspace).mockResolvedValue(baseState);
    ...
    const aliceRevision = {
      ...baseRevision,
      revision: "alice",
      path: "migrations/versions/alice.py",
      downRevisions: ["base"],
      isCurrent: false,
    };
    const bobRevision = {
      ...baseRevision,
      revision: "bob",
      path: "migrations/versions/bob.py",
      downRevisions: ["base"],
      isCurrent: false,
    };
    const branchedState = (revisionNode: typeof aliceRevision): WorkspaceState => ({
      ...baseState,
      files: [...baseState.files, revisionNode.path],
      revisions: [revisionNode, { ...baseRevision, isHead: false }],
    });
    const integrationStates = [
      branchedState(aliceRevision),
      {
        ...baseState,
    ...
    vi.mocked(integrationClient.writeFile)
      .mockResolvedValueOnce({
        state: integrationStates[0],
        fileChanges: [{ path: aliceRevision.path, change: "added" }],
      })
      .mockResolvedValueOnce({
        state: integrationStates[1],
        fileChanges: [{ path: bobRevision.path, change: "added" }],
      });
    const factory = vi
      .fn()
      .mockReturnValueOnce(source)
      .mockReturnValueOnce(aliceClient)
      .mockReturnValueOnce(bobClient)
      .mockReturnValueOnce(integrationClient);
    const lab = createLab(factory);
    await lab.create("sqlite");
    ...
    expect(lab.store.getState().workspaces.map((workspace) => workspace.role)).toEqual([
      "base",
      "alice",
      "bob",
      "integration",
    ]);
    lab.store.setState((state) => ({
      workspaces: state.workspaces.map((workspace) =>
        workspace.id === collaboration.aliceId
          ? { ...workspace, snapshot: branchedState(aliceRevision) }
          : workspace.id === collaboration.bobId
            ? { ...workspace, snapshot: branchedState(bobRevision) }
            : workspace,
      ),
    }));
    ...
    for (const runtime of [source, aliceClient, bobClient, integrationClient])
