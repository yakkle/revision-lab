...
describe("Lab workspace orchestration", () => {
  ...
  it("deletes a revision through the runtime, removes its draft, and checkpoints the returned graph", async () => {
    ...
    const revisionState: WorkspaceState = { ...snapshot, files: ["models.py", revisionPath], revisions: [{
      revision: "r1", path: revisionPath, downRevisions: [], branchLabels: [], dependsOn: [],
      isHead: true, isBranchPoint: false, isMergePoint: false, isCurrent: false,
    }] };
    const runtime = client();
    const persistence = repository();
    vi.mocked(runtime.createWorkspace).mockResolvedValue(revisionState);
    vi.mocked(runtime.readFile).mockImplementation(async (path) => path === revisionPath ? "revision = 'r1'" : "model");
    vi.mocked(runtime.deleteRevision).mockResolvedValue({ state: snapshot, fileChanges: [{ path: revisionPath, change: "deleted" }] });
    ...
    expect(lab.store.getState().workspaces[0]).toMatchObject({ snapshot, file: "models.py", revision: undefined,
  ...
  it("restores runtime files, database seed, lesson evidence, and the selected workspace from IndexedDB metadata", async () => {
    ...
    const checkpoint: WorkspaceCheckpoint = {
      ...
      workspace: { id: "sqlite-saved", name: "저장된 SQLite", mode: "sqlite", evidence: {
        ...
        downgradeObserved: false, manualUsersUpgradeObserved: false, manualUsersDowngradeObserved: false,
        autogeneratedRevisions: [], reviewedRevisions: [], autogenerateBaselineObserved: false, autogenerateApplied: false,
