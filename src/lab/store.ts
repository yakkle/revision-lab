...
export type LabClient = Pick<AlembicRuntimeClient, "createWorkspace" | "exportClone" | "readFile" | "writeFile" | "deleteRevision" | "runCommand" | "readTable" |
...
const evidenceKeys: Array<keyof LessonEvidence> = [
  ...
  "manualUsersUpgradeObserved", "manualUsersDowngradeObserved", "autogeneratedRevisions", "reviewedRevisions",
  "autogenerateBaselineObserved", "autogenerateApplied", "multipleHeadsErrorObserved",
];
...
export function createLab(
  ...
  const workspaceFromCheckpoint = (record: WorkspaceCheckpoint, snapshot?: WorkspaceState): Workspace => ({
    ...
    savedAt: record.savedAt, evidence: { ...emptyLessonEvidence(), ...record.workspace.evidence }, role: record.workspace.role,
    ...
    deleteRevision(path: string) {
      return perform(async (workspace, client) => {
        if (Object.values(workspace.drafts).some((draft) => draft.text !== draft.saved)) {
          throw new RuntimeClientError({ code: "UNSAVED_CHANGES", message: "저장하지 않은 편집을 먼저 저장하세요." });
        }
        ...
        const evidence = deletedRevision ? {
          ...workspace.evidence,
          autogeneratedRevisions: workspace.evidence.autogeneratedRevisions.filter((revision) => revision !== deletedRevision),
          reviewedRevisions: workspace.evidence.reviewedRevisions.filter((revision) => revision !== deletedRevision),
        } : workspace.evidence;
        patch(workspace.id, { snapshot: result.state, changes: result.fileChanges, stale: false, data: undefined,
          drafts, file: undefined, revision: undefined, evidence });
        if (fallback) {
